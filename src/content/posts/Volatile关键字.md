---
title: "Volatile关键字"
description: "1 内存模型 缓存不一致问题 计算机在执行程序时，每条指令都是在CPU中执行的，而执行指令过程中，势必涉及到数据的读取和写入。由于程序运行过程中的临时数据是存放在主存（物理内存）当中的，这时就存在一个问题，由于CPU执行速度很快，而从内存读取数据和向内存写入数据的过程跟CPU执行指令的速度比起来要慢的多，因此如果任何时"
pubDatetime: 2021-08-11T16:00:00.000Z
modDatetime: 2021-08-11T16:00:00.000Z
categories:
  - "技术"
tags:
  - "Java"
